...
评测结果（300 道题，人工评分，0/0.5/1 三档）：图谱增强整体平均分 **0.867**，纯向量基线 **0.75**；在「实体关系类」问题上从 0.75 提升到 **1.00**，提升最明显（见下文「评测结果」）。
...
`eval/` 目录下 300 道题（语义类 / 实体关系类 / 混合类各 100 道），对比「纯向量 RAG」与「图谱增强 RAG（完整 workflow）」，人工按 0 / 0.5 / 1 三档打分：

| 问题类型 | 纯向量 RAG | 图谱增强 RAG |
|---|---|---|
| 语义类（100 题） | 0.750 | 0.750 |
| 实体关系类（100 题） | 0.750 | **1.000** |
| 混合类（100 题） | 0.750 | 0.850 |
| **整体平均** | **0.750** | **0.867** |
